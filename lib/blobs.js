...
var basepath= __dirname + "/../storage/Blobs/";
var compressedbasepath= __dirname + "/../storage/compressedBlobs/";
...
exports.getCompressed = function(sha){
    return fs.readFileAsync(getCompressedPath(sha));
};
...
exports.save = function(sha, blob){
    ...
    return createDirectories()
        .then(function(){
            ...
            return zlib.gzipAsync(blob);
        }).then(function(compressedblob){
            return fs.writeFileAsync(getCompressedPath(sha), compressedblob);
        });
};
...
function getCompressedPath(sha){
    return compressedbasepath + sha[0] + sha[1] + "/" + sha; 
}
...
function createDirectories(){
    ...
    for (var i = 0; i < 256; i++) {
        ...
        promises.push(mkdirp(compressedbasepath + directoryName));
    }
